...
def sharpe(returns, rf=0.0, days=252):
    ...
    # print("mean: {}".format(returns.mean()))
...
class Trader:
    ...
    def __init__(self, config, data_loader, df):
        ...
        try:
            ...
            self.date_indices = self.original_df.index[self.trade_period_nxt_day]
    ...
    def calc_return(self, torch_output_list):
        ...
        self.buy_and_hold_return, self.buy_and_hold_ret_list, self.buy_and_hold_daily_ret_list = self.buy_and_hold()
        reg_return, self.reg_ret_lists, self.reg_daily_ret_list = self.trade_with_regression_result()
        # reg_return, reg_ret_list = self.trade_with_regression_result_5day()

    def get_sharpe(self, ret_a=None, ret_b=None):
        if not ret_a:
            ret_a = self.buy_and_hold_daily_ret_list
        if not ret_b:
            ret_b = self.reg_daily_ret_list
        sharpe_a = sharpe(ret_a)
        sharpe_b = sharpe(ret_b)
        return sharpe_a, sharpe_b

    def plot_ret(self, plot=True):
        if plot:
            ReturnPlotter(self.date_indices, [self.buy_and_hold_ret_list, self.reg_ret_list], ["Buy&Hold", "CNNLSTM"], self.get_sharpe(self.buy_and_hold_daily_ret_list, self.reg_daily_ret),
    ...
    def buy_and_hold(self):
        ...
        for cur_day, nxt_day in zip(self.cur_day_value, self.nxt_day_value):
            ret_list.append(nxt_day/beginning_price)
            daily_ret_list.append((nxt_day-cur_day)/cur_day)
        buy_and_hold_return = (self.nxt_day_value[-1] - beginning_price) / beginning_price
        return buy_and_hold_return, ret_list, daily_ret_list
    ...
    def trade_with_regression_result(self, short=False):
        ...
        for cur_day, nxt_day, decision in zip(self.cur_day_value, self.nxt_day_value, self.decision_output):
            ...
            daily_ret_list.append(earned_pct)
        ...
        if total_earned > self.tot_earned:
            ...
            self.reg_daily_ret = daily_ret_list
        return total_earned, ret_list, daily_ret_list
